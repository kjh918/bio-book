...
def apply_content_block(slide, shape, block: ContentBlock):
    ...
    Priority when multiple fields are set: image_path > plot > table >
    dataframe > bullets > paragraphs. `block.position`/`block.size` (inches)
    optionally override where the image/table is placed; bullets/paragraphs
    always use the shape's own text box."""
    if block is None:
        return
    ...
    elif block.plot is not None:
        stream = su.render_plot_to_stream(block.plot, dpi=block.plot_dpi)
        su.fill_image(slide, shape, stream, box=box)
    elif block.table:
        su.fill_table(slide, shape, block.table.headers, block.table.rows, box=box)
    elif block.dataframe is not None:
        headers, rows = su.dataframe_to_table(block.dataframe)
        su.fill_table(slide, shape, headers, rows, box=box)
    elif block.bullets:
