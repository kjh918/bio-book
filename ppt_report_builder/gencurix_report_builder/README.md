...
`[CONTENTS]` 라고 쓰인 자리는 전부 이 객체 하나로 처리됩니다. 직접 `ContentBlock(...)`을
만들어도 되고, 아래 **`Image()` / `Plot()` / `Table()` 헬퍼 함수**를 쓰면 더 자연스럽게
쓸 수 있습니다 (내부적으로 전부 `ContentBlock`을 반환합니다). 값은 하나만 채우세요
(우선순위: `image_path` > `plot` > `table` > `dataframe` > `bullets` > `paragraphs`):

```python
from gencurix_report import Image, Plot, Table

ResultItem(title="...", content=Image("chart.png"))          # 이미 저장된 PNG/JPG 경로
ResultItem(title="...", content=Plot(fig))                    # 저장 안 한 matplotlib Figure/Axes를 그대로
ResultItem(title="...", content=Table(dataframe=df))          # 저장 안 한 pandas DataFrame을 그대로
ResultItem(title="...", content=Table(headers=[...], rows=[[...]]))  # 순수 리스트로 표
ContentBlock(bullets=["항목1", "항목2"])                        # 불릿 리스트
ContentBlock(paragraphs=["문단1", "문단2"])                     # 불릿 없는 문단
```

**`Plot(figure)` / `Table(dataframe=...)`는 파일로 미리 저장할 필요가 없습니다** —
분석 코드에서 만든 `fig`, `df` 객체를 그대로 넘기면 빌드 시점에 default 양식으로
자동 변환됩니다:

- `Plot(fig, dpi=200)` → 흰 배경 / `bbox_inches="tight"` / 200dpi로 메모리에서 바로
  PNG 렌더링해서 삽입 (디스크에 임시파일 안 만듦). 차트 자체의 색상/스타일은 건드리지
  않고, **내보내기 설정(해상도·여백·배경)만 일관된 default를 적용**합니다.
- `Table(dataframe=df)` → 컬럼명을 헤더로, 각 행을 문자열로 변환해서 표로 삽입.
  float 값은 기본적으로 유효숫자 3자리(`{:.3g}`)로 포맷됩니다
  (`headers, rows = slide_utils.dataframe_to_table(df, float_format="{:.2f}")`처럼
  직접 변환 함수를 불러써서 포맷을 바꿀 수도 있습니다).
- `Image(path)` / `Table(headers=..., rows=...)` 는 기존처럼 파일 경로/순수 리스트를
  그대로 씁니다.

- `image_path` / `plot`: 원래 placeholder의 위치/크기 박스에 맞춰 **비율을 유지하며**
  자동 축소/중앙정렬되어 삽입됩니다.
- `table` / `dataframe`: PowerPoint 네이티브 표로 삽입됩니다 (템플릿 테이블 스타일 적용).
- `bullets`/`paragraphs`: 템플릿의 기존 글머리 기호·폰트·크기 서식을 그대로 복제해서 줄 수만큼 문단을 생성합니다.

`position`/`size`는 `Image()`/`Plot()`/`Table()` 헬퍼에도 그대로 전달할 수 있습니다:

```python
Plot(fig, position=(0.7, 1.6), size=(8.5, 3.8))
```

(YAML 설정 파일에서는 살아있는 Python 객체를 담을 수 없으므로, YAML 경로에서는 계속
`image:`/`table:` 의 `path` 방식만 지원합니다 -- `Plot`/`Table(dataframe=...)` 는
Python API 전용입니다.)
...
또는 최상위에 있는 `generate_report.py` 스크립트로 (더 친절한 에러 메시지,
`--dry-run` 검증, 커스텀 템플릿 지정 지원):
...
python generate_report.py report_config.yaml --dry-run     # pptx 생성 없이 YAML/파일경로만 검증
```

`generate_report.py`는:
- YAML 파싱 실패, 템플릿/설정 파일 없음, PNG/TSV 경로 오류를 실행 전에 미리 잡아서
  알려줍니다 (`--dry-run`으로 실제 생성 없이 검증만 할 수도 있음).
- 성공/실패에 따라 exit code 0/1/2를 반환하므로 쉘 스크립트나 파이프라인에 바로
  넣어 쓸 수 있습니다.
- `-v`/`--verbose`를 주면 에러 발생 시 전체 traceback을 출력합니다.
